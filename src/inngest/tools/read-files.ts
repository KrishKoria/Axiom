...
export function createReadFilesTool({ internalKey }: ReadFilesToolOptions) {
  return createTool({
    name: "read-files",
    description:
      "Reads the contents of one or more files in the project. Useful for understanding code or data stored in files. Returns the contents of the files.",
    parameters: z.object({
    ...
    handler: async (params, { step: toolStep }) => {
      ...
      if (!parsed.success) {
        throw new Error(`Error: ${parsed.error.issues[0].message}`);
      }
